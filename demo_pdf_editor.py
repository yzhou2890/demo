...
def extract_single_page( pdfpath="", pdfname="", pagelist=[], pdfname_out="" ):   
    # extract a specific page from a pdf file

    pdfReader   = PyPDF2.PdfFileReader(open(pdfpath+pdfname+".pdf", "rb"))
    pdf_writer  = PyPDF2.PdfFileWriter()

    pdf_writer.addPage(pdfReader.getPage(pagelist-1))      # '-1' is here as page numbers start from 0, not 1
...
if 0:
    ...
    pdfname = "marks"               # filenmae - no extension
...
if 1:
    # demo of making lots of single page pdf files
    pdfpath = ""                    # file path 
    pdfname = "marks"               # filenmae - no extension
    pagelist = [1,2,3,4,5,6,7]      # page num starts from 1 till end, note this is different from Python's view
    namelist = ["LiL","FengZY","LiuSH","ZhangJN","GavaliM","OuWT","IslamB"]
    for i in range(len(pagelist)):
        extract_single_page( pdfpath, pdfname, pagelist[i], pdfname_out = namelist[i])
