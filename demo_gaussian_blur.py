...
#----------
# demo of Gaussian blur to webcam frames
#----------
...
#    vcap.set(cv2.CAP_PROP_EXPOSURE, 0.001)         #set exposure time
    ...
    while(True):
        ...
        '''Gaussian filtering'''
        r = cv2.GaussianBlur( rgb[:,:,0], (9,9),0)
        g = cv2.GaussianBlur( rgb[:,:,1], (9,9),0)
        b = cv2.GaussianBlur( rgb[:,:,2], (9,9),0)
        
        '''X'''
        X = np.vstack((rgb,cv2.merge((r,g,b))))
        cv2.putText(X,'raw image',(10,30),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,0,0),1,cv2.LINE_AA)
        cv2.putText(X,'Gaussian blurred',(10,nheight+30),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,0,0),1,cv2.LINE_AA)

        '''show'''
        cv2.imshow('gaussian blur - To quit, press \'q\'', X)
